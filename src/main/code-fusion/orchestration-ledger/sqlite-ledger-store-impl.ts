...
export class SqliteOrchestrationLedgerStore implements OrchestrationLedgerStore {
  ...
  readAfter(options: OrchestrationLedgerReadOptions = {}): readonly OrchestrationLedgerRecord[] {
    ...
    `).all(afterSequence, limit) as unknown as LedgerRow[]).map(rowToRecord)
  }
  ...
  readAggregate(
    ...
    this.assertOpen()
    const type = requireAggregateType(aggregateType)
    const id = requireText('aggregateId', aggregateId, MAX_ID_LENGTH)
    const { afterSequence, limit } = normalizeReadOptions(options)
    return (this.database.prepare(`
      SELECT * FROM code_fusion_ledger_events
      WHERE aggregate_type = ? AND aggregate_id = ? AND sequence > ?
      ORDER BY sequence ASC LIMIT ?
    `).all(type, id, afterSequence, limit) as unknown as LedgerRow[]).map(rowToRecord)
  }
  ...
  readProject(
    ...
    this.assertOpen()
    const id = requireText('projectId', projectId, MAX_ID_LENGTH)
    const { afterSequence, limit } = normalizeReadOptions(options)
    return (this.database.prepare(`
      SELECT * FROM code_fusion_ledger_events
      WHERE project_id = ? AND sequence > ? ORDER BY sequence ASC LIMIT ?
    `).all(id, afterSequence, limit) as unknown as LedgerRow[]).map(rowToRecord)
  }
  ...
  private migrate(): void {
    ...
    if (version === ORCHESTRATION_LEDGER_SCHEMA_VERSION) return
    if (version !== 0) throw new Error(`No Code Fusion ledger migration path from schema ${version}`)
  ...
  private assertOpen(): void {
    if (this.closed) throw new Error('Code Fusion orchestration ledger is closed')
  }
}
...
function serializeEvent(event: OrchestrationLedgerEventInput): SerializedEvent {
  ...
  const normalized: OrchestrationLedgerEventInput = {
    ...
    ...(projectId ? { projectId } : {}),
    ...(event.actor ? { actor: { kind: requireActorKind(event.actor.kind), ...(actorId ? { id: actorId } : {}) } } : {}),
    ...(source ? { source } : {}),
  ...
  if (payloadJson === undefined) throw new Error('Code Fusion ledger payload must be JSON serializable')
...
function rowToRecord(row: LedgerRow): OrchestrationLedgerRecord {
  return {
    ...
    ...(row.actor_kind ? { actor: { kind: row.actor_kind, ...(row.actor_id ? { id: row.actor_id } : {}) } } : {}),
...
function normalizeReadOptions(options: OrchestrationLedgerReadOptions): { afterSequence: number; limit: number } {
...
function requireAggregateType(value: OrchestrationLedgerAggregateType): OrchestrationLedgerAggregateType {
  if (!AGGREGATE_TYPES.has(value)) throw new Error(`Unsupported Code Fusion ledger aggregate type: ${String(value)}`)
  return value
}

function requireActorKind(value: OrchestrationLedgerActorKind): OrchestrationLedgerActorKind {
  if (!ACTOR_KINDS.has(value)) throw new Error(`Unsupported Code Fusion ledger actor kind: ${String(value)}`)
  return value
}

function requireText(name: string, value: string, maxLength: number): string {
  if (typeof value !== 'string') throw new Error(`Code Fusion ledger ${name} must be text`)
...
function assertJsonValue(value: unknown, depth = 0, ancestors: Set<object> = new Set()): asserts value is JsonValue {
  if (depth > MAX_PAYLOAD_DEPTH) throw new Error(`Code Fusion ledger payload exceeds maximum depth ${MAX_PAYLOAD_DEPTH}`)
  if (value === null || typeof value === 'string' || typeof value === 'boolean') return
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw new Error('Code Fusion ledger payload numbers must be finite')
    return
  }
  if (typeof value !== 'object') throw new Error('Code Fusion ledger payload must contain JSON values only')
  if (ancestors.has(value)) throw new Error('Code Fusion ledger payload must not contain cycles')
  ancestors.add(value)
  try {
    if (Array.isArray(value)) {
      for (const item of value) assertJsonValue(item, depth + 1, ancestors)
      return
    }
